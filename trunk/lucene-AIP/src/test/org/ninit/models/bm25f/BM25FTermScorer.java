...
public class BM25FTermScorer extends Scorer {
    ...
    /**
     * Storing in "termDocs", for each field, an array of the docId that the
     * term "term" appears
     * 
     * @param reader
     * @param term
	...
	this.bParam = bParams;
	/*
	 * AIP Comment: "termDocs" es un array donde cada posición corresponde a
	 * un field, y cada una de esas posiciones contendrá otro array que
	 * seran los documentos donde el término aparece (para ese campo)
	 */

	this.termDocs = new TermDocs[this.fields.length];
	// AIP Comment: guarda un booleando en el que, para cada campo dice si
	// hay mas documentos o no
	...
	try {
	    ...
	    // idf value only depends on the term frequency in the doc, this
	    // stats is given by Lucene
	    // AIP comment: el valor que se pasa para calcular el idf es el
	    // docFreq del campo catchAll
	    this.idf = this.getSimilarity().idf(
		    this.reader.docFreq(new Term(Constants.CATCHALL_FIELD, term
     ...
     */
    /*
     * @Override public Explanation explain(int doc) throws IOException { if
     * (this.advance(doc) == NO_MORE_DOCS) return null;
     * 
     * float acum = 0f; Explanation result = new Explanation(); Explanation tf =
     * new Explanation(); result.setDescription("BM25F (" +
     * this.term.getTerm().text() + ")");
     * 
     * for (int i = 0; i < this.fields.length; i++) {
     * 
     * if (this.termDocs[i].doc() == doc) { Explanation partial = new
     * Explanation();
     * 
     * byte[] norm = this.reader.norms(this.fields[i]);
     * 
     * float av_length = (float) BM25FParameters
     * .getAverageLength(this.fields[i]); float length = 1 /
     * ((Similarity.decodeNorm(norm[this.doc()])) * (Similarity
     * .decodeNorm(norm[this.doc()])));
     * 
     * float aux = 0f; aux = this.bParam[i] * length / av_length;
     * 
     * aux = aux + 1 - this.bParam[i]; acum += this.boost[i] *
     * this.termDocs[i].freq() / aux;
     * 
     * partial = new Explanation(this.boost[i] this.termDocs[i].freq() / aux,
     * "(" + this.fields[i] + ":" + this.term.getTerm().text() + ") B:" +
     * this.bParam[i] + ",Length:" + length + ",AvgLength:" + av_length +
     * ",Freq:" + this.termDocs[i].freq() + ",Boost:" + this.boost[i]);
     ...
     * this.term.getTerm().text())) + ",numDocs:" + this.reader.numDocs() +
     * ")"); result.addDetail(idfE);
     * 
     * tf.setDescription("K1: " + acum + "/(" + acum + " + " +
     * BM25FParameters.getK1() + ")");
     * 
     * acum = acum / (BM25FParameters.getK1() + acum); tf.setValue(acum);
     * result.addDetail(tf); acum = acum * this.idf; result.setValue(acum);
     * 
     * return result; }
     */
    ...
    /*
     * Hace el computo del score sumando los score de cada campo
     * 
     ...
     * 
     * @Override public boolean skipTo(int target) throws IOException { while
     * (this.doc() < target && this.next()) { }
     * 
