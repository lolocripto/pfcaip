...
public class BM25BooleanScorer extends Scorer {
     ...
     *//*
        ...
        * result.setDescription("Total"); Explanation detail = null; float value
        * = 0f; if (this.hasMoreMust && this.mustBooleanScorer.docID() == doc)
        * detail = this.mustBooleanScorer.explain(doc); if (detail != null) {
        * result.addDetail(this.mustBooleanScorer.explain(doc)); value +=
        * detail.getValue(); }
        * 
        * if (this.hasMoreShould && this.shouldBooleanScorer.docID() == doc)
        * detail = this.shouldBooleanScorer.explain(doc); if (detail != null) {
        * result.addDetail(this.shouldBooleanScorer.explain(doc)); value +=
        * detail.getValue(); } result.setValue(value); return result; }
        */
	...
	while (this.doc < this.ndocs - 1) {
		...
		if (this.mustBooleanScorer.docID() == this.notBooleanScorer
			.docID())
		    return this.docID();
	    } else
		return NO_MORE_DOCS;
