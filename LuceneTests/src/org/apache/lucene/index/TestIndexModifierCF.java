 ...
 * @deprecated
 */
public class TestIndexModifierCF extends LuceneTestCase {
  ...
  public void testExtendedIndex() throws IOException {
    ...
    assertEquals(10, powerIndex.colFreq(allDocTerm));
    powerIndex.close();
  }
  ...
  private Document getDoc() {
    ...
    doc.add(new Field("all", "x", Field.Store.YES, Field.Index.NOT_ANALYZED));//adding double of 'x'
    docCount++;
    return doc;
  }
  ...
  private void testIndexInternal(int maxWait) throws Exception {
    ...
    IndexThreadCF.id = 0;
    IndexThreadCF.idStack.clear();
    IndexModifier index = new IndexModifier(rd, new StandardAnalyzer(), create);
    IndexThreadCF thread1 = new IndexThreadCF(index, maxWait, 1);
    thread1.start();
    IndexThreadCF thread2 = new IndexThreadCF(index, maxWait, 2);
    thread2.start();
  ...
  private class PowerIndex extends IndexModifier {
    ...
    public int colFreq(Term term) throws IOException{
        synchronized(directory) {
            assureOpen();
            createIndexReader();
            return indexReader.colDocFreq(term);
          }
    }
  }
...
class IndexThreadCF extends Thread {
  ...
  IndexThreadCF(IndexModifier index, int maxWait, int threadNumber) {
    this.index = index;
