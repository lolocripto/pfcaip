...
public class TestDoc extends LuceneTestCase {
    ...
    public void testIndexAndMerge() throws Exception {
      ...
      printSegment(out, si1, "test.txt");

      SegmentInfo si2 = indexDoc(writer, "test2.txt");
      printSegment(out, si2, "test2.txt");
      writer.close();

      SegmentInfo siMerge = merge(si1, si2, "merge", false);
      printSegment(out, siMerge,"merge(si1, si2, merge, false);");

      SegmentInfo siMerge2 = merge(si1, si2, "merge2", false);
      printSegment(out, siMerge2,"merge(si1, si2, merge2, false);");

      SegmentInfo siMerge3 = merge(siMerge, siMerge2, "merge3", false);
      printSegment(out, siMerge3,"merge(siMerge, siMerge2, merge3, false)");
      ...
      printSegment(out, siMerge,"merge(si1, si2, merge, true)");

      siMerge2 = merge(si1, si2, "merge2", true);
      printSegment(out, siMerge2,"merge(si1, si2, merge2, true)");

      siMerge3 = merge(siMerge, siMerge2, "merge3", true);
      printSegment(out, siMerge3,"merge(si1, si2, merge3, true)");
   ...
   private void printSegment(PrintWriter out, SegmentInfo si, String text)
   throws Exception {
      SegmentReader reader = SegmentReader.get(si);

      for (int i = 0; i < reader.numDocs(); i++)
        out.println(reader.document(i) + "["+text+"]");

      TermEnum tis = reader.terms();
      while (tis.next()) {
        out.println(tis.term());
        out.println(" DF=" + tis.docFreq());
        out.println(" CF=" + tis.colFreq());
